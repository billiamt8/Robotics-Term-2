...
void setup()
  ...
  void ButtonPressed() 
  {
    if (millis() - lastDebounceTime >= debounceDelay)  // wait for debounce time
    {
      buttonState = digitalRead(btnPin);  // read btn state
      if (buttonState != lastButtonState)  // check btn state
      {
